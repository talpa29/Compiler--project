...
let rec gcd a b =
  match (a, b) with 0, b -> b | a, 0 -> a | a, b -> gcd b (a mod b)
...
module Reader : READER = struct
  ...
  type string_part = Static of string | Dynamic of sexpr

  let unitify nt = pack nt (fun _ -> ())

  let rec nt_whitespace str = const (fun ch -> ch <= ' ') str
  ...
  and nt_paired_comment str =
    let nt_c = char '{' in
    let nt_mid = star nt_sexpr in
    let nt_c_e = char '}' in
    let nt_caten = caten nt_c (caten nt_mid nt_c_e) in
    let nt_comment = unitify nt_caten in
    nt_comment str

  and nt_sexpr_comment str =
    let nt1 = word "#;" in
    let nt2 = caten nt1 nt_sexpr in
    let nt3 = unitify nt2 in
    nt3 str
  ...
  and nt_digit str =
    ...
    let nt_ascii_48 = int_of_char '0' in
    let nt_val = pack nt_digit (fun ch -> int_of_char ch - nt_ascii_48) in
    nt_val str

  and nt_hex_digit str =
    let letters = range_ci 'a' 'f' in
    let nt_val = pack letters (fun c -> int_of_char c - 87) in
    let nt_hex_dig = disj nt_digit nt_val in
    nt_hex_dig str

  and nt_nat str =
    let nt_num = plus nt_digit in
    let nt_val =
      pack nt_num (fun digs ->
          List.fold_left (fun acc dig -> (acc * 10) + dig) 0 digs)
    in
    nt_val str
  ...
  and nt_hex_nat str =
    ...
    let nt1 =
      pack nt1 (fun digits ->
          List.fold_left (fun num digit -> (16 * num) + digit) 0 digits)
    in
    nt1 str
  ...
  and nt_optional_sign str =
    ...
    let nt_op_sign = maybe nt_sign in
    let nt_bool_sign =
      pack nt_op_sign (fun res ->
          match res with
          | Some false -> false
          | Some true -> true
          | None -> true)
  ...
  and nt_int str =
    ...
    let nt1 =
      pack nt1 (fun (is_positive, n) -> if is_positive then n else -n)
    in
    nt1 str
  ...
  and nt_frac str =
    ...
    let nt1 = caten nt1 nt2 in
    let nt1 =
      pack nt1 (fun (num, den) ->
          let d = gcd num den in
          match d < 0 with
          | true ->
              let d = -d in
              ScmRational (num / d, den / d)
          | _ -> ScmRational (num / d, den / d))
    in
    nt1 str
  ...
  and num_to_list d =
    let rec dig acc d =
      if d < 10 then d :: acc else dig ((d mod 10) :: acc) (d / 10)
    in
    dig [] d
  ...
  and nt_floatA str =
    ...
    let nt2 = char '.' in
    let nt3 =
      pack (maybe nt_mantissa) (fun b ->
          match b with None -> 0. | Some number -> number)
    in
    let nt4 = caten nt1 (caten nt2 nt3) in
    let nt5 = pack nt4 (fun (num, (dot, mantis)) -> num +. mantis) in
    let nt6 =
      pack (maybe nt_exponent) (fun exp ->
          match exp with None -> 0. | Some number -> number)
    in
    let nt7 = caten nt5 nt6 in
    let nt8 =
      pack nt7 (fun (num, exp) ->
          match exp with 0. -> ScmReal num | _ -> ScmReal (num *. exp))
    in
    nt8 str
  ...
  and nt_floatB str =
    ...
    let nt5 =
      pack (maybe nt_exponent) (fun exp ->
          match exp with None -> 0. | Some number -> number)
    in
    let nt6 = caten nt4 nt5 in
    let nt7 =
      pack nt6 (fun ((dot, num), exp) ->
          match exp with 0. -> ScmReal num | _ -> ScmReal (num *. exp))
    in
    nt7 str
  ...
  and nt_floatC str =
    ...
    let nt4 = pack nt3 (fun (num, exp) -> ScmReal (num *. exp)) in
    nt4 str

  and nt_float str =
    let nt4 = disj (disj nt_floatA nt_floatB) nt_floatC in
    let nt5 =
      pack nt4 (fun num -> match num with ScmReal number -> number | _ -> 0.0)
    in
    let nt6 =
      pack (caten nt_optional_sign nt5) (fun (sign, num) ->
          match sign with true -> ScmReal num | false -> ScmReal (-1. *. num))
    in
    nt6 str
  ...
  and nt_symbol str =
    ...
    let nt3 = pack nt2 (fun s -> ScmSymbol s) in
    nt3 str
    ...
    let nt1 =
      disj_list
        [
          ...
          unitify (char '\\');
          unitify (word "~~");
          unitify nt_string_part_dynamic;
        ]
    in
  ...
  and nt_string_part_dynamic str =
    ...
    let nt5 =
      pack nt4 (fun ((_, sexp), _) ->
          ScmPair
            ( ScmSymbol "format",
              ScmPair (ScmString "~a", ScmPair (sexp, ScmNil)) ))
    in
    let nt6 = pack nt5 (fun res -> Dynamic res) in
    nt6 str

  and nt_string_part_static str =
    let nt1 =
      disj_list
        [ nt_string_part_simple; nt_string_part_meta; nt_string_part_hex ]
    in
    let nt1 = plus nt1 in
  ...
  and nt_string_part str = disj nt_string_part_static nt_string_part_dynamic str
          ...
          | parts ->
              let argl =
                List.fold_right
                  ...
                  parts ScmNil
              in
              ScmPair (ScmSymbol "string-append", argl))
    in
    nt1 str

  and nt_empty_vector str =
    let nt1 = caten nt_skip_star (word "#(") in
    let nt2 = caten nt1 nt_skip_star in
    let nt3 = caten nt2 (char ')') in
    let nt4 = caten nt3 nt_skip_star in
    let nt5 = pack nt4 (fun _ -> ScmVector []) in
    nt5 str

  and nt_vector str =
    let nt1 = pack (caten nt_skip_star (word "#(")) (fun (_, w) -> w) in
    let nt2 = star nt_sexpr in
    let nt3 = pack (caten (char ')') nt_skip_star) (fun (_, c) -> c) in
    let nt4 = caten (caten nt1 nt2) nt3 in
    let nt5 = pack nt4 (fun ((_, sexpr), _) -> ScmVector sexpr) in
    let nt6 = disj nt_empty_vector nt5 in
    nt6 str
  ...
  and nt_list str =
    ...
    let nt5 =
      pack
        (caten (char '.') (caten nt_sexpr (char ')')))
        (fun (_, (sexpr, _)) -> sexpr)
    in
    let nt4 = disj nt4 nt5 in
    let nt3 =
      pack (caten nt3 nt4) (fun (sexprs, sexpr) ->
  ...
  and make_quoted_form nt_qf qf_name =
    ...
    let nt1 =
      pack nt1 (fun (_, sexpr) ->
          ScmPair (ScmSymbol qf_name, ScmPair (sexpr, ScmNil)))
    in
    nt1
  ...
  and nt_quoted_forms str =
    let nt1 =
      disj_list
        [
          make_quoted_form (unitify (char '\'')) "quote";
          make_quoted_form (unitify (char '`')) "quasiquote";
          make_quoted_form
            (unitify (not_followed_by (char ',') (char '@')))
            "unquote";
          make_quoted_form (unitify (word ",@")) "unquote-splicing";
        ]
    in
    nt1 str
...
let rec string_of_sexpr = function
        ...
        (String.concat ""
           (List.map
              (function
                ...
                | '\"' -> "\\\""
                | ch ->
                    if ch < ' ' then Printf.sprintf "\\x%x;" (int_of_char ch)
                    else Printf.sprintf "%c" ch)
              (list_of_string str)))
  | ScmSymbol sym -> sym
  ...
  | ScmNumber (ScmRational (num, -1)) -> Printf.sprintf "%d" (-num)
  | ScmNumber (ScmRational (num, den)) -> Printf.sprintf "%d/%d" 5 5
  ...
  | ScmPair (ScmSymbol "unquote-splicing", ScmPair (sexpr, ScmNil)) ->
      Printf.sprintf ",@%s" (string_of_sexpr sexpr)
  | ScmPair (car, cdr) -> string_of_sexpr' (string_of_sexpr car) cdr

and string_of_sexpr' car_string = function
  | ScmNil -> Printf.sprintf "(%s)" car_string
  | ScmPair (cadr, cddr) ->
      let new_car_string =
        Printf.sprintf "%s %s" car_string (string_of_sexpr cadr)
      in
...
let sprint_sexprs chan sexprs =
  Printf.sprintf "[%s]" (String.concat "; " (List.map string_of_sexpr sexprs))
...
type lambda_kind = Simple | Opt of string
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec scheme_list_to_ocaml = function
    | ScmNil -> ([], ScmNil)
    | ScmPair (car, cdr) ->
        (fun (rdc, last) -> (car :: rdc, last)) (scheme_list_to_ocaml cdr)
    | rac -> ([], rac)
  ...
  let list_contains_unquote_splicing =
    ormap (function
      | ScmPair (ScmSymbol "unquote-splicing", ScmPair (_, ScmNil)) -> true
      | _ -> false)
  ...
  let rec macro_expand_qq = function
    ...
    | ScmPair (ScmPair (ScmSymbol "unquote", ScmPair (car, ScmNil)), cdr) ->
        let cdr = macro_expand_qq cdr in
        ScmPair (ScmSymbol "cons", ScmPair (car, ScmPair (cdr, ScmNil)))
    | ScmPair
        (ScmPair (ScmSymbol "unquote-splicing", ScmPair (sexpr, ScmNil)), ScmNil)
      ->
        sexpr
    | ScmPair
        (ScmPair (ScmSymbol "unquote-splicing", ScmPair (car, ScmNil)), cdr) ->
        let cdr = macro_expand_qq cdr in
    ...
    | ScmPair (car, cdr) ->
        ...
        ScmPair (ScmSymbol "cons", ScmPair (car, ScmPair (cdr, ScmNil)))
    | ScmVector sexprs ->
        if list_contains_unquote_splicing sexprs then
          let sexpr =
            macro_expand_qq (scheme_sexpr_list_of_sexpr_list sexprs)
          in
          ScmPair (ScmSymbol "list->vector", ScmPair (sexpr, ScmNil))
        else
          let sexprs =
            scheme_sexpr_list_of_sexpr_list (List.map macro_expand_qq sexprs)
          in
  ...
  let rec macro_expand_cond_ribs ribs =
    ...
    | ScmPair
        (ScmPair (expr, ScmPair (ScmSymbol "=>", ScmPair (func, ScmNil))), ribs)
      ->
                              ...
                              ( ScmSymbol "rest",
                                ScmPair
                                  ( ScmPair
                                      ( ScmSymbol "lambda",
                                        ScmPair
                                          (ScmNil, ScmPair (remaining, ScmNil))
                                      ),
                                    ScmNil ) ),
                            ScmNil ) ) ),
                ScmPair
                          ...
                          ( ScmSymbol "value",
                            ScmPair
                              ( ScmPair
                                ...
                                ScmPair
                                  (ScmPair (ScmSymbol "rest", ScmNil), ScmNil)
                              ) ) ),
                    ScmNil ) ) )
  ...
  let rec names_values = function
    ...
    | ScmPair (ScmPair (arg, ScmPair (value, ScmNil)), ScmNil) ->
        ([ arg ], [ value ])
    | ScmPair (ScmPair (arg, ScmPair (value, ScmNil)), rest_ribs) ->
        let names, values = names_values rest_ribs in
        (arg :: names, value :: values)
    | _ -> raise (X_this_should_not_happen "names_values")
  ...
  let rec tag_parse sexpr =
    ...
    | ScmSymbol var ->
        if is_reserved_word var then
          raise (X_syntax "Variable cannot be a reserved word")
    ...
    | ScmPair (ScmSymbol "set!", ScmPair (ScmSymbol var, ScmPair (expr, ScmNil)))
      ->
        if is_reserved_word var then
          raise (X_syntax "cannot assign a reserved word")
                 ...
                 ( var,
                   ScmPair
                     ( ScmPair (ScmSymbol "lambda", ScmPair (params, exprs)),
                       ScmNil ) ) ))
    | ScmPair
        (ScmSymbol "define", ScmPair (ScmSymbol var, ScmPair (expr, ScmNil))) ->
        if is_reserved_word var then
          raise (X_syntax "cannot define a reserved word")
    ...
    | ScmPair (ScmSymbol "let", ScmPair (ScmNil, exprs)) ->
        tag_parse
          (ScmPair
             (ScmPair (ScmSymbol "lambda", ScmPair (ScmNil, exprs)), ScmNil))
    | ScmPair (ScmSymbol "let", ScmPair (ribs, exprs)) ->
        let names, values = names_values ribs in
        let names = scheme_sexpr_list_of_sexpr_list names in
        let values = scheme_sexpr_list_of_sexpr_list values in
        tag_parse
    ...
    | ScmPair
        ( ScmSymbol "let*",
          ScmPair
            (ScmPair (ScmPair (var, ScmPair (value, ScmNil)), ScmNil), exprs) )
      ->
        tag_parse
          (ScmPair
             ( ScmSymbol "let",
    ...
    | ScmPair
        ...
        tag_parse
          (ScmPair
             ( ScmSymbol "let",
               ScmPair
                 ( ScmPair (ScmPair (var, ScmPair (arg, ScmNil)), ScmNil),
                   ScmPair
                     (ScmPair (ScmSymbol "let*", ScmPair (ribs, exprs)), ScmNil)
                 ) ))
    | ScmPair (ScmSymbol "letrec", ScmPair (ScmNil, exprs)) ->
        tag_parse (ScmPair (ScmSymbol "let", ScmPair (ScmNil, exprs)))
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
        ...
        let set_bangs =
          List.map2
            (fun name value ->
              ScmPair (ScmSymbol "set!", ScmPair (name, ScmPair (value, ScmNil))))
            names values
        in
        let exprs =
          List.fold_right
            (fun set_bangs exprs -> ScmPair (set_bangs, exprs))
            set_bangs exprs
        in
        tag_parse (ScmPair (ScmSymbol "let", ScmPair (whatever_ribs, exprs)))
    ...
    | ScmPair (ScmSymbol "and", ScmPair (expr, exprs)) ->
        ...
        tag_parse
          (ScmPair
             ( ScmSymbol "if",
               ScmPair (expr, ScmPair (rest, ScmPair (ScmBoolean false, ScmNil)))
             ))
    ...
    | ScmPair (ScmSymbol "or", exprs) ->
        let rec or_exprs_list = function
          | ScmPair (car, ScmNil) -> [ tag_parse car ]
          | ScmPair (car, cdr) -> [ tag_parse car ] @ or_exprs_list cdr
          | _ -> raise (X_this_should_not_happen "or")
        in
        ScmOr (or_exprs_list exprs)
    ...
    | ScmPair (proc, args) -> (
        let proc =
          ...
          | ScmSymbol var ->
              if is_reserved_word var then
                raise (X_syntax "reserved word in proc position")
              else proc
          | proc -> proc
        in
        match scheme_list_to_ocaml args with
        | args, ScmNil -> ScmApplic (tag_parse proc, List.map tag_parse args)
        | _ -> raise (X_syntax "malformed application"))
    | sexpr ->
        raise
          (X_syntax (Printf.sprintf "Unknown form: \n%a\n" sprint_sexpr sexpr))
end

(* end of struct Tag_Parser *)

type app_kind = Tail_Call | Non_Tail_Call
type lexical_address = Free | Param of int | Bound of int * int
...
(* end of signature SEMANTIC_ANALYSIS *)

module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  let rec lookup_in_rib name = function
    | [] -> None
    | name' :: rib -> (
        if name = name' then Some 0
        else
  ...
  let rec split_last_elem_ocaml_list = function
    | car :: [] -> ([], car)
    | car :: cdr ->
        (fun (rdc, last) -> (car :: rdc, last)) (split_last_elem_ocaml_list cdr)
    | _ -> raise (X_this_should_not_happen "split_last_elem_ocaml_list")
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmVarSet (Var v, expr) ->
          ScmVarSet'
            (tag_lexical_address_for_var v params env, run expr params env)
      ...
      | ScmLambda (params', Opt opt, expr) ->
          let actual_params = params' @ [ opt ] in
          let new_env = params :: env in
          ScmLambda' (params', Opt opt, run expr actual_params new_env)
      | ScmApplic (proc, args) ->
          ScmApplic'
            ( run proc params env,
  ...
  let annotate_tail_calls =
    ...
    in
    fun expr' -> run false expr' (*Check again*)
  ...
  let combine_pairs =
    List.fold_left (fun (rs1, ws1) (rs2, ws2) -> (rs1 @ rs2, ws1 @ ws2)) ([], [])
  ...
  let find_reads_and_writes =
    let rec run name expr params env =
      ...
      | ScmSeq' exprs ->
          combine_pairs (List.map (fun expr -> run name expr params env) exprs)
      ...
      | ScmApplic' (proc, args, app_kind) ->
          ...
          let rs2, ws2 =
            combine_pairs (List.map (fun arg -> run name arg params env) args)
          in
          (rs1 @ rs2, ws1 @ ws2)
    in
    fun name expr params -> run name expr params []

  let cross_product as' bs' =
    List.concat (List.map (fun ai -> List.map (fun bj -> (ai, bj)) bs') as')
  ...
  let should_box_var name expr params =
    ...
    if List.length reads = 0 || List.length writes = 0 then false
      ...
      let rec run = function
        | [] -> false
        | ((Var' (v1, Param _), _), (Var' (v2, Param _), _)) :: rest -> run rest
        | ((Var' (v1, Param _), _), (Var' (v2, Bound _), _)) :: rest -> true
        | ((Var' (v1, Bound _), _), (Var' (v2, Param _), _)) :: rest -> true
        | ((Var' (v1, Bound _), env2), (Var' (v2, Bound _), env1)) :: rest ->
            (not (find_var_rib name env1 == find_var_rib name env2)) || run rest
        | _ :: rest -> run rest
      and find_var_rib name = function
        | [] -> raise (X_this_should_not_happen "find_var_rib")
        | rib :: env ->
            if List.mem name rib then (rib : string list)
  ...
  let box_sets_and_gets name body =
    let rec run expr =
      ...
      | ScmVarGet' (Var' (name', _) as v) ->
          if name = name' then ScmBoxGet' v else expr
      | ScmBox' _ -> expr
      | ScmBoxGet' _ -> expr
      ...
      | ScmVarSet' ((Var' (_, Free) as v), expr') -> ScmVarSet' (v, run expr')
      | ScmVarSet' ((Var' (name', _) as v), expr') ->
          if name = name' then ScmBoxSet' (v, run expr')
  ...
  let make_sets =
    let rec run minor names params =
      ...
      | name :: names', param :: params' ->
          if name = param then
            let v = Var' (name, Param minor) in
            ScmVarSet' (v, ScmBox' v) :: run (minor + 1) names' params'
          else run (minor + 1) names params'
      | _, _ ->
          raise (X_this_should_not_happen "no free vars should be found here")
    in
  ...
  let rec auto_box expr =
    ...
    | ScmVarSet' (v, expr) -> ScmVarSet' (v, auto_box expr) (*not sure*)
    | ScmVarDef' (v, expr) -> ScmVarDef' (v, auto_box expr) (*not sure*)
    | ScmOr' exprs -> ScmOr' (List.map (fun exp -> auto_box exp) exprs)
    | ScmLambda' (params, Simple, expr') ->
        let box_these =
          List.filter (fun param -> should_box_var param expr' params) params
        in
        let new_body =
          List.fold_left
            (fun body name -> box_sets_and_gets name body)
            (auto_box expr') box_these
        in
    ...
    | ScmLambda' (params, Opt opt, expr') ->
        ...
        let box_these =
          List.filter
            (fun param -> should_box_var param expr' actual_params)
            actual_params
        in
        let new_body =
          List.fold_left
            ...
            (auto_box expr') box_these
        in
        let new_sets = make_sets box_these actual_params in
        let new_body =
          match (box_these, new_body) with
          | [], _ -> new_body
          | _, ScmSeq' exprs -> ScmSeq' (new_sets @ exprs)
  ...
  let semantics expr =
    auto_box (annotate_tail_calls (annotate_lexical_address expr))
end
...
let rec sexpr_of_expr' = function
  ...
  | ScmLambda' (params, Simple, expr) ->
      ...
      let params =
        Reader.scheme_sexpr_list_of_sexpr_list
          (List.map (fun str -> ScmSymbol str) params)
      in
  ...
  | ScmApplic' (proc, args, app_kind) ->
      ...
      let args =
        Reader.scheme_sexpr_list_of_sexpr_list (List.map sexpr_of_expr' args)
      in
      ScmPair (proc, args)
  | ScmOr' exprs ->
      ScmPair
  ...
  | _ -> raise (X_not_yet_implemented "it's ok")

let string_of_expr' expr =
  Printf.sprintf "%a" sprint_sexpr (sexpr_of_expr' expr)

let print_expr' chan expr = output_string chan (string_of_expr' expr)
...
let string_of_expr expr = Printf.sprintf "%a" sprint_sexpr (sexpr_of_expr expr)
let print_expr chan expr = output_string chan (string_of_expr expr)
...
module type CODE_GENERATION = sig
  ...
  val remove_duplicates : sexpr list -> sexpr list
...
module Code_Generation : CODE_GENERATION = struct
  ...
  let list_and_last =
    let rec run a = function
      ...
      | b :: s ->
          ...
          (a :: s, last)
    in
    function [] -> None | a :: s -> Some (run a s)

  let split_to_sublists n =
    let rec run = function
      | [], _, f -> [ f [] ]
      | s, 0, f -> f [] :: run (s, n, fun s -> s)
      | a :: s, i, f -> run (s, i - 1, fun s -> f (a :: s))
    in
    function [] -> [] | s -> run (s, n, fun s -> s)

  let remove_duplicates lst =
    let remove_dups xs =
      List.fold_left
        (fun xs x -> if List.mem x xs then xs else xs @ [ x ])
        [] xs
    in
    remove_dups lst
  ...
  let collect_constants =
    let rec run = function
      ...
      | ScmSeq' lst -> List.fold_left (fun acc curr -> acc @ run curr) [] lst
      | ScmOr' lst -> List.fold_left (fun acc curr -> acc @ run curr) [] lst
      ...
      | ScmApplic' (expr', exprs', _) ->
          run expr' @ List.fold_left (fun acc curr -> acc @ run curr) [] exprs'
    in
    fun exprs' -> List.fold_left (fun acc expr' -> acc @ run expr') [] exprs'
      ...
      | [] ->
          raise
            (X_this_should_not_happen
               (Printf.sprintf "The const was not found in the constant table"))
      | (sexpr, loc, repr) :: _ when s = sexpr -> loc
      | _ :: table -> run s table
    in
    run
  ...
  let const_repr sexpr loc table =
    ...
    | ScmNil -> ([ RTTI "T_nil" ], 1)
    | ScmBoolean false -> ([ RTTI "T_boolean_false" ], 1)
    | ScmBoolean true -> ([ RTTI "T_boolean_true" ], 1)
    | ScmChar ch -> ([ RTTI "T_char"; Byte (int_of_char ch) ], 2)
    | ScmString str ->
        let count = String.length str in
        ([ RTTI "T_string"; Quad count; ASCII str ], 1 + word_size + count)
    | ScmSymbol sym ->
    ...
    | ScmNumber (ScmRational (numerator, denominator)) ->
        ( [ RTTI "T_rational"; Quad numerator; Quad denominator ],
          1 + (2 * word_size) )
    | ScmNumber (ScmReal x) -> ([ RTTI "T_real"; QuadFloat x ], 1 + word_size)
    | ScmVector s ->
        let addrs =
          List.map (fun si -> ConstPtr (search_constant_address si table)) s
        in
        let count = List.length s in
        (RTTI "T_vector" :: Quad count :: addrs, 1 + ((count + 1) * word_size))
    | ScmPair (car, cdr) ->
        let addr_car, addr_cdr =
          (search_constant_address car table, search_constant_address cdr table)
        in
  ...
  let make_constants_table =
    ...
    fun exprs' ->
      run [] 0
        (remove_duplicates
           (add_sub_constants (remove_duplicates (collect_constants exprs'))))

  let asm_comment_of_sexpr sexpr =
    let str = string_of_sexpr sexpr in
    let str =
      if String.length str <= comment_length then str
    ...
    let run = function
      ...
      | [ RTTI "T_string"; Quad length; ASCII const_str ] ->
          Printf.sprintf "\tdb T_string\t%s\n\tdq %d%s" str length
            (let s = list_of_string const_str in
             let s =
               List.map (fun ch -> Printf.sprintf "0x%02X" (int_of_char ch)) s
             in
          ...
          Printf.sprintf "\tdb T_vector\t%s\n\tdq %d%s" str length
            (let s =
               List.map
                 (function
                   | ConstPtr ptr ->
                       Printf.sprintf "%s + %d" label_start_of_constants_table
                         ptr
                   | _ ->
                       raise
                         (X_this_should_not_happen
      ...
      | [ RTTI "T_pair"; ConstPtr car; ConstPtr cdr ] ->
          Printf.sprintf "\tdb T_pair\t%s\n\tdq %s + %d, %s + %d" str
            label_start_of_constants_table car label_start_of_constants_table
            cdr
  ...
  let asm_of_constants_table =
    ...
    in
    fun table ->
      Printf.sprintf "%s:\n%s" label_start_of_constants_table (run table)
  ...
  let collect_free_vars =
    ...
    fun exprs' ->
      let primitives =
        List.map (fun (scheme_name, _) -> scheme_name) global_bindings_table
  ...
  let make_free_vars_table =
    ...
    in
    fun exprs' -> run 0 (collect_free_vars exprs')
    ...
    let rec run v = function
      | [] ->
          raise
            (X_this_should_not_happen
               (Printf.sprintf
                  "The variable %s was not found in the free-var table" v))
  ...
  let make_lambda_opt_create_list_loop =
    make_make_label ".L_lambda_opt_create_list_loop"

  let make_lambda_opt_create_list_loop_a =
    make_make_label ".L_lambda_opt_create_list_loop_a"

  let make_lambda_opt_create_list_loop_b =
    make_make_label ".L_lambda_opt_create_list_loop_b"

  let make_lambda_opt_exact = make_make_label ".L_lambda_opt_exact"

  let make_lambda_opt_exact_end = make_make_label ".L_lambda_opt_exact_end"
  let make_lambda_opt_stack_ok = make_make_label ".L_lambda_opt_stack_adjusted"

  let make_lambda_opt_loop_shrink =
    make_make_label ".L_lambda_opt_stack_shrink_loop"

  let make_lambda_opt_loop_shrink_end = 
    make_make_label ".L_lambda_opt_stack_shrink_loop_end"

  let make_lambda_opt_loop_expand =
    make_make_label ".L_lambda_opt_stack_expand_loop"
  ...
  let make_lambda_opt_arity_exact = make_make_label ".L_lambda_opt_arity_exact"
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      | ScmConst' sexpr ->
          let rel_addr = search_constant_address sexpr consts in
          Printf.sprintf "\t;get const %s\n" (string_of_sexpr sexpr)
      ...
      | ScmVarGet' (Var' (v, Param minor)) ->
          Printf.sprintf ";BEGIN of var param get %s, minor: %d\n" v minor
          ^ Printf.sprintf "\t;get param var: %s, minor: %d\n" v minor
          ^ Printf.sprintf "\tmov rax, qword [rbp + 8 * (4 + %d)]\n" minor
          ^ Printf.sprintf ";END of var param get %s, minor: %d\n" v minor
      | ScmVarGet' (Var' (v, Bound (major, minor))) ->
          Printf.sprintf ";BEGIN of bound var get %s, major: %d, minor: %d\n" v
            major minor
          ^ Printf.sprintf "\tmov rax, ENV\n"
          ^ Printf.sprintf "\tmov rax, qword [rax + 8 * %d]\n" major
          ^ Printf.sprintf "\tmov rax, qword [rax + 8 * %d]\n" minor
          ^ Printf.sprintf ";END of bound var get %s, major: %d, minor: %d\n" v
              major minor
      | ScmIf' (test, dit, dif) ->
          let label_else = make_if_else ()
          and label_end = make_if_end ()
          and test_code = run params env test
          and dit_code = run params env dit
          and dif_code = run params env dif in
          Printf.sprintf ";BEGIN of if statment\n"
          ^ Printf.sprintf "%s" test_code
          ^ Printf.sprintf "\tcmp rax, sob_boolean_false\n"
          ^ Printf.sprintf "\tje %s\n" label_else
          ^ Printf.sprintf "%s" dit_code
          ^ Printf.sprintf "\tjmp %s\n" label_end
          ^ Printf.sprintf "%s:\n" label_else
          ^ Printf.sprintf "%s" dif_code
          ^ Printf.sprintf "%s:" label_end
          ^ Printf.sprintf ";END of if statment\n"
      ...
      | ScmVarDef' (Var' (v, Param minor), expr') ->
          raise (X_this_should_not_happen "does not support nested define")
      | ScmVarDef' (Var' (v, Bound (major, minor)), expr') ->
          raise (X_this_should_not_happen "does not support nested define")
      | ScmBox' (Var' (v, Param minor)) ->
          Printf.sprintf ";Begin of gen box\n"
          ^ Printf.sprintf "\tmov rbx, PARAM(%d)\t; get param val\n" minor
          ^ Printf.sprintf "\tmov rdi, 8\n"
          ^ Printf.sprintf "\tcall malloc\n"
          ^ Printf.sprintf "\tmov qword [rax], rbx\n"
          ^ Printf.sprintf ";End of gen box\n"
      | ScmBox' _ ->
          raise
            (X_this_should_not_happen
               "ScmBox made only on params (look at auto_box func)")
      | ScmBoxGet' var' ->
          run params env (ScmVarGet' var') ^ "\tmov rax, qword [rax]\n"
      | ScmBoxSet' (var', expr') ->
          (*need to check if works*)
          Printf.sprintf ";BEGIN of box set\n"
          ^ Printf.sprintf "%s" (run params env expr')
          ^ Printf.sprintf "\tpush rax\n"
          ^ Printf.sprintf "%s" (run params env (ScmVarGet' var'))
          ^ Printf.sprintf "\tpop qword [rax]\n"
          ^ Printf.sprintf "\tmov rax, sob_void\n"
          ^ Printf.sprintf ";END of box set\n"
      | ScmLambda' (params', Simple, body) ->
          ...
          and label_end = make_lambda_simple_end () in
          Printf.sprintf "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
          ^ Printf.sprintf "\tcall malloc\n"
          ^ Printf.sprintf "\tpush rax\n"
          ...
          ^ Printf.sprintf "\tjmp %s\n" label_loop_params
          ^ Printf.sprintf "%s:\n" label_loop_params_end
          ^ Printf.sprintf "\tmov qword [rax], rbx\t; ext_env[0] <-- new_rib \n"
          ...
          ^ Printf.sprintf "\tjmp L_error_incorrect_arity_simple\n"
          ^ Printf.sprintf "%s:\n" label_arity_ok
          ^ Printf.sprintf "\tenter 0, 0\n"
          ^ Printf.sprintf "%s" (run (List.length params') (env + 1) body)
      ...
      | ScmLambda' (params', Opt opt, body) ->
          ...
          and label_exact_loop = make_lambda_opt_exact ()
          and label_exact_loop_end = make_lambda_opt_exact_end ()
          and label_stack_ok = make_lambda_opt_stack_ok ()
          and label_shrink_loop = make_lambda_opt_loop_shrink ()
          and label_shrink_loop_end = make_lambda_opt_loop_shrink_end ()
          and label_create_list = make_lambda_opt_create_list_loop ()
          and label_create_list_end = make_lambda_opt_create_list_loop_a ()
          and label_arity_ok_more = make_lambda_opt_arity_more () in
          Printf.sprintf ";BEGIN of gen lambda opt\n"
          ^ Printf.sprintf "\tmov rdi, (1 + 8 + 8)\t; sob closure\n"
          ^ Printf.sprintf "\tcall malloc\n"
          ^ Printf.sprintf "\tpush rax\n"
          ^ Printf.sprintf "\tmov rdi, 8 * %d\t; new rib\n" params
          ...
          ^ Printf.sprintf "%s:\n" label_arity_exact
          ^ Printf.sprintf "\tmov rdi, rsp\n"
          ^ Printf.sprintf "\tsub rdi, 8*1\n"
          ^ Printf.sprintf "\tmov rax, qword[ rdi + 8*1]\n"
          ^ Printf.sprintf "\tmov [rdi], rax\n"
          ^ Printf.sprintf "\tadd rdi, 8*1\n"
          ^ Printf.sprintf "\tmov rax, qword[ rdi + 8*1]\n"
          ^ Printf.sprintf "\tmov [rdi], rax\n"
          ^ Printf.sprintf "\tadd rdi, 8*1\n"
          ^ Printf.sprintf "\tmov rax, qword [rdi + 8*1]\n"
          ^ Printf.sprintf "\tmov rcx, rax\n"
          ^ Printf.sprintf "\tinc rax\n"
          ^ Printf.sprintf "\tmov qword [rdi], rax\n"
          ^ Printf.sprintf "\tadd rdi, 8*1\n"
          ^ Printf.sprintf "%s:\n" label_exact_loop
          ^ Printf.sprintf "\tcmp rcx, 0\n"
          ^ Printf.sprintf "\tje %s\n" label_exact_loop_end
          ^ Printf.sprintf "\tmov rax, qword [rdi + 8]\n"
          ^ Printf.sprintf "\tmov qword [rdi], rax\n"
          ^ Printf.sprintf "\tadd rdi, 8*1\n"
          ^ Printf.sprintf "\tdec rcx\n"
          ^ Printf.sprintf "\tjmp %s\n" label_exact_loop
          ^ Printf.sprintf "%s:\n"label_exact_loop_end
          ^ Printf.sprintf "\tmov qword[rdi], sob_nil\n"
          ^ Printf.sprintf "\tsub rsp, 8*1\n"
          ^ Printf.sprintf "\tjmp %s\n" label_stack_ok
          ^ Printf.sprintf "%s:\t; have variadic args\n" label_arity_ok_more
          ^ Printf.sprintf "\tmov rsi, qword[rsp + 8*2]\t;pointing on the count of args\n"
          ^ Printf.sprintf "\tlea rcx, [rsi - %d]\t; init counter\n" (List.length params')
          ^ Printf.sprintf "\tlea rsi, [rsp + 8*rsi + 8*2]\t; point to the top of the stack\n"
          ^ Printf.sprintf "\tmov r10, rsi\t; backup regiter\n"
          ^ Printf.sprintf "\tmov r11, rsi\t; backup regiter\n"
          ^ Printf.sprintf "\tmov r9, sob_nil\n"
          ^ Printf.sprintf "%s:\n" label_create_list
          ^ Printf.sprintf "\tcmp rcx, 0\n"
          ^ Printf.sprintf "\tje %s\n"label_create_list_end
          ^ Printf.sprintf "\tmov rdi, 1 + 8 + 8\n"
          ^ Printf.sprintf "\tcall malloc\n"
          ^ Printf.sprintf "\tmov byte [rax], T_pair \n"
          ^ Printf.sprintf "\tmov rbx, qword [rsi]\n"
          ^ Printf.sprintf "\tmov SOB_PAIR_CAR(rax), rbx\n"
          ^ Printf.sprintf "\tmov SOB_PAIR_CDR(rax), r9\n"
          ^ Printf.sprintf "\tmov r9, rax\n"
          ^ Printf.sprintf "\tsub rsi, 8*1\n"
          ^ Printf.sprintf "\tdec rcx\n"
          ^ Printf.sprintf "\tjmp %s\n" label_create_list
          ^ Printf.sprintf "%s:\n" label_create_list_end
          ^ Printf.sprintf "\tmov qword [r10], r9\t; putting the list in the top of the stack\n"
          ^ Printf.sprintf "\tsub r10, 8*1\t; pointer to the first param to change in the stack\n"
          ^ Printf.sprintf "\tlea rsi, [rsp + 8*(%d + 2)]\t; minus one for the last parameter because we stack from zero\n" (List.length params')
          ^ Printf.sprintf "\tmov rcx, %d\t; init counter for loop\n" (List.length params')
          ^ Printf.sprintf "%s:\n" label_shrink_loop
          ^ Printf.sprintf "\tcmp rcx, 0\n"
          ^ Printf.sprintf "\tje %s\n" label_shrink_loop_end
          ^ Printf.sprintf "\tmov r9, qword[rsi]\n"
          ^ Printf.sprintf "\tmov qword [r10], r9\n"
          ^ Printf.sprintf "\tsub r10, 8*1\n"
          ^ Printf.sprintf "\tsub rsi, 8*1\n"
          ^ Printf.sprintf "\tdec rcx\n"
          ^ Printf.sprintf "\tjmp %s\n" label_shrink_loop
          ^ Printf.sprintf "%s:\n" label_shrink_loop_end
          ...
          ^ Printf.sprintf "\tsub r10, 8*1\n"
          ^ Printf.sprintf "\tsub rsi, 8*1\n"
          ^ Printf.sprintf "\tmov rax, qword [rsi]\t; env\n"
          ^ Printf.sprintf "\tmov qword[r10], rax\n"
          ^ Printf.sprintf "\tsub r10, 8*1\n"
          ^ Printf.sprintf "\tsub rsi, 8*1\n"
          ^ Printf.sprintf "\tmov rax, qword [rsi]\t; ret address\n"
          ^ Printf.sprintf "\tmov qword[r10],rax\n"
          ^ Printf.sprintf "\tmov rsp, r10\n"
          ^ Printf.sprintf "%s:\n" label_stack_ok
          ^ "\tenter 0, 0\n"
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) ->
          (*need to check if works*)
          Printf.sprintf ";BEGIN of applic call - non_tail_call\n"
          ^ Printf.sprintf "%s"
              (List.fold_right
                 (fun curr acc -> acc ^ run params env curr ^ "\tpush rax\n")
                 args "")
          ^ Printf.sprintf "\tpush %d\n" (List.length args)
          ^ Printf.sprintf "%s" (run params env proc)
          ^ Printf.sprintf "\tassert_closure(rax)\n"
          ^ Printf.sprintf "\tpush SOB_CLOSURE_ENV(rax)\n"
          ^ Printf.sprintf "\tcall SOB_CLOSURE_CODE(rax)\n"
          ^ Printf.sprintf ";END of applic call - non_tail_call\n"
      | ScmApplic' (proc, args, Tail_Call) ->
          ...
          Printf.sprintf ";BEGIN of applic call - tail_call\n"
          ^ Printf.sprintf "%s" args_code
          ^ Printf.sprintf "\tpush %d\t; push args count\n" (List.length args)
          ^ Printf.sprintf "%s" proc_code
          ^ Printf.sprintf "\tassert_closure(rax)\n"
          ^ Printf.sprintf "\tpush SOB_CLOSURE_ENV(rax)\n"
          ^ Printf.sprintf "\t;recycling current frame\n"
          ^ Printf.sprintf
              "\tpush qword [rbp + 8 * 1]\t; save old return addr\n"
          ^ Printf.sprintf "\tpush qword [rbp + 8 * 0]\t; save old rbp\n"
          ^ Printf.sprintf "\tmov rcx, %d + 4\t; size of new frame \n"
              (List.length args)
          ^ Printf.sprintf "\tmov rbx, COUNT\t; num of args \n"
          ^ Printf.sprintf
              "\tlea rbx, [rbp + 8 * rbx + 8 * 3]\t; address of old frame \
               beginning \n"
          ^ Printf.sprintf
              "\tlea rdx, [rbp - 8 * 1]\t; address of new frame beginning (we \
               have two frames on stack before the recycling)\n\n"
          ^ Printf.sprintf
              "\t; explanation of the next steps: we have two pointers, rbx \
               (pointer for old frame), rdx (pointer for new frame), \n"
          ^ Printf.sprintf
              "\t; we will loop and copy the date from [rdx] to [rbx] and sub \
               both rdx and rbx (make them point to the next cell in stack)\n\n"
          ^ Printf.sprintf "%s:\n" label_recycle_frame_loop
          ^ Printf.sprintf "\tcmp rcx, 0\t; check if finish loop on new frame\n"
          ^ Printf.sprintf "\tje %s\n" label_recycle_frame_done
          ^ Printf.sprintf
              "\tmov rsi, qword [rdx]\t; save data of current cell in new frame\n"
          ^ Printf.sprintf
              "\tmov qword [rbx], rsi\t; overwrite the data on the cell rbx \
               points to\n"
          ^ Printf.sprintf
              "\tdec rcx\t; counter for the new frame size, when rcx = 0 we \
               stop the loop\n"
          ^ Printf.sprintf
              "\tsub rbx, 8\t; promoting rbx to the next cell in old frame \n"
          ^ Printf.sprintf
              "\tsub rdx, 8\t; promoting rdx to the next cell in new frame \n"
          ^ Printf.sprintf "\tjmp %s\n" label_recycle_frame_loop
          ^ Printf.sprintf "%s:\n" label_recycle_frame_done
          ^ Printf.sprintf
              "\tlea rsp, [rbx + 8 * 1]\t; rbx points to one cell after the \
               last cell in the new frame we made, we need to move rsp one \
               place back\n"
          ^ Printf.sprintf
              "\tpop rbp\t; rsp points to the cell that holds the old rbp we \
               pushed before, the applied proc will restore it (push it back \
               to stack)\n"
          ^ Printf.sprintf "\tjmp SOB_CLOSURE_CODE(rax)\n"
          ^ Printf.sprintf ";END of applic call - tail_call\n"
