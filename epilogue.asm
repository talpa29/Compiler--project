...
L_code_ptr_bin_apply:
        mov rbx, rsp                           ; demo rbp
        cmp qword [rbx + 8 * 2], 2      
        jne L_error_arg_count_2
        mov r10, 0                              ; list length counter
        mov r8, qword [rbx + 8 * 3]             ; the function
        assert_closure(r8)
        mov r9, qword [rbx + 8 * 4]             ; the list
        mov r11, r9                             ; r11 also holds pointer to the list
.L_count_list_loop:
        cmp qword r11, sob_nil
        je .L_count_list_loop_end
        assert_pair(r11)
        mov r11, SOB_PAIR_CDR(r11)
        inc r10
        jmp .L_count_list_loop
.L_count_list_loop_end:
        lea rsp, [rbx + 8 * 4]          ; rsp points to the beggining of old frame
        lea r11, [8 * r10]
        sub rsp, r11                    ; rsp points beginning of old frame + (8 * list length)
        add rsp, 8 
        mov r11, rsp
        mov rcx, [rbx]                  ; rcx holds the ret addr
        push r10                        ; add number of args to frame (list length)
        push SOB_CLOSURE_ENV(r8)        ; push env of f
        push rcx                        ; save ret addr of apply since f in tail pos
.L_push_list_loop:
        cmp qword r9, sob_nil
        je .L_push_list_loop_end
        mov r12, SOB_PAIR_CAR(r9)
        mov [r11], r12                  ; pushing curr elem of list to stack in reverse way
        mov r9, SOB_PAIR_CDR(r9)
        add r11, 8                      ; r11 points to next cell
        jmp .L_push_list_loop
.L_push_list_loop_end:
